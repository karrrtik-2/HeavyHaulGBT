...
# User input history storage - will store up to 5 previous user inputs only
user_input_history = []
MAX_HISTORY = 4
MAX_PERMIT_HISTORY = 3

# Function to log conversation to file
...
# Function to add to user input history only
def update_user_input_history(user_input):
    user_input_history.append(user_input)
    
    # Keep only last 4 user inputs
    while len(user_input_history) > MAX_HISTORY:
        user_input_history.pop(0)

# Function to get previous user inputs for LLM context
def get_previous_user_inputs(limit=MAX_HISTORY):
    # Return list of messages with only user inputs
    messages = []
    
    # Get the appropriate number of previous inputs
...
def ask_llm(question):
    ...
    # Add previous user inputs for context (up to 4)
    messages.extend(get_previous_user_inputs())
    
    # Add current question
    messages.append({"role": "user", "content": question})
    
    #print("Messages sent to ask_LLM:", json.dumps(messages, ensure_ascii=False))
...
def process_user_query(user_input, order_id, order_document):
    ...
    # Add user input to history first (before processing)
    update_user_input_history(user_input)
    
    # Check for state mention in user input and update global_state_value
    ...
    #print(f"here is the results which are being passed to llm: {restructured_data}")
    ...
    if has_permit_info and mentioned_state and 'routeData' in restructured_data:
        ...
        if state_name and state_permit_info:
            ...
            if order_id:
                order_dimensions = fetch_order_data(order_id)
