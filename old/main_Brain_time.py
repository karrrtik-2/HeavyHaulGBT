...
def ask_llm(question):
    ...
    print(f"[TIMING] ask_llm - API: {api_duration:.2f}s, Total: {total_duration:.2f}s")
...
def generate_final_response(user_input, restructured_data):
    ...
    print(f"[TIMING] generate_final_response - API: {api_duration:.2f}s, Total: {total_duration:.2f}s")
...
def process_user_query(user_input, order_id, order_document):
    ...
    llm_response, llm_timing = ask_llm(user_input)
    ...
    if 'download' in user_input_lower:
        ...
        if download_messages:
            ...
            timing_info = {
                "total_process_time": f"{process_duration:.2f}s",
                "llm_api_time": llm_timing["api_duration"],
                "llm_total_time": llm_timing["total_duration"]
            }
            
            # Log the conversation with timing
            timing_str = f"Total: {timing_info['total_process_time']}, LLM API: {timing_info['llm_api_time']}, LLM Total: {timing_info['llm_total_time']}"
            log_conversation(user_input, response_text, timing_str)
            
            print(f"[TIMING] Total process time: {process_duration:.2f}s")
    ...
    if has_permit_info and mentioned_state and 'routeData' in restructured_data:
        ...
        if state_name and state_permit_info:
            ...
            permit_api_start_time = time.time()
            stream = groq_client.chat.completions.create(
                model="llama-3.3-70b-specdec",
                messages=messages,
                temperature=0.0,
            ...
            permit_api_end_time = time.time()
            
            permit_api_duration = permit_api_end_time - permit_api_start_time
            ...
            timing_info = {
                ...
                "llm_api_time": llm_timing["api_duration"],
                "llm_total_time": llm_timing["total_duration"],
                "permit_api_time": f"{permit_api_duration:.2f}s"
            }
            
            # Log the conversation with timing
            timing_str = f"Total: {timing_info['total_process_time']}, LLM API: {timing_info['llm_api_time']}, LLM Total: {timing_info['llm_total_time']}, Permit API: {timing_info['permit_api_time']}"
            log_conversation(user_input, response_text, timing_str)
    ...
    final_response, final_timing = generate_final_response(user_input, restructured_data)
    ...
    timing_info = {
        ...
        "llm_api_time": llm_timing["api_duration"],
        "llm_total_time": llm_timing["total_duration"],
        "final_response_api_time": final_timing["api_duration"],
        "final_response_total_time": final_timing["total_duration"]
    }
    
    # Log the conversation with timing
    timing_str = f"Total: {timing_info['total_process_time']}, Initial LLM API: {timing_info['llm_api_time']}, Final Response API: {timing_info['final_response_api_time']}"
    log_conversation(user_input, final_response, timing_str)
...
async def voice_assistant(order_id, order_document):
    ...
    while True:
        ...
        listen_duration = listen_end_time - process_start_time
        ...
        print(f"[TIMING] Voice processing - Listen: {listen_duration:.2f}s, Speech: {speech_duration:.2f}s, Total: {total_duration:.2f}s")
...
def main():
    ...
    print(f"here si the order data loaded:{order_document}")
